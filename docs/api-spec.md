...
| PATCH  | `/players/me/profile/avatar`         | Bearer | `UpdateAvatarSchema`           | Updates only the player avatar (preset id or cropped image data URL). Requires an existing profile; avoids resubmitting the whole profile.                                                                       |
| PATCH  | `/matches/:id/frames/:frameNumber`   | Bearer | `UpdateMatchFrameSchema`       | Edits a frame's scores/break/duration/notes (or full `scoreEvents` log); re-derives the winner from the scores/log and recomputes the match score/result. Returns the updated match.                             |
| DELETE | `/matches/:id/frames/last`           | Bearer | —                              | Removes the last (highest-numbered) frame and recomputes the match score/result. No-op when there are no frames. Returns the updated match.                                                                      |
...
| DELETE | `/drill-executions/:id/attempts/last`| Bearer | —                              | Removes the last attempt of an open execution and decrements counters in a serializable transaction. No-op (returns the execution) when there are no attempts. Used by the training UI's "undo attempt" button. |
...
| POST   | `/matches`                           | Bearer | `CreateMatchSchema`            | Creates a manual current-player match or sparring (`matchType`) with opponent, score, location, break and key-stat fields.                                                                                       |
| PATCH  | `/matches/:id`                       | Bearer | `UpdateMatchSchema`            | Updates manual match/sparring metadata (incl. `matchType`), score and key stats for a current-player match.                                                                                                      |
| POST   | `/matches/:id/frames`                | Bearer | `AddMatchFrameSchema`          | Adds one frame in a serializable transaction with retry and recalculates frame score/result from saved frames. Accepts an optional `scoreEvents` log; when present the frame totals/winner are recomputed from it via the snooker-domain engine. |
